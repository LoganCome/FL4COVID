...
class CNN(nn.Module):
    def __init__(self, classes, model='resnet18'):
        ...
        elif (model == 'resnext50_32x4d' or model == 'resneXt'):
            self.cnn = models.resnext50_32x4d(pretrained=False)
            self.cnn.classifier = nn.Linear(1280, classes)
        elif (model == 'mobilenet2'):
            self.cnn = models.mobilenet_v2(pretrained=False)
            self.cnn.classifier = nn.Linear(1280, classes)
        elif (model == 'densenet169'):
            self.cnn = models.densenet169(pretrained=False, num_classes=classes)
